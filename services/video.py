...
# ядра. Значение подобрано под хост с 4 ядрами: два транскода по два потока
# занимают все четыре, но при nice 19 процесс бота вытесняет их без задержки.
#
# На одноядерном хосте это значение обязано быть 1, а лучше фичу там вовсе не
# включать: уступать процессор некуда, второго ядра для бота просто нет.
...
#
# 2 потока на транскод при двух параллельных заданиях = 4 потока на 4 ядрах.
# Менять вместе с _transcode_slots: важно их произведение, а не каждое
