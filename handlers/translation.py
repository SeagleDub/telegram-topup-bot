...
async def translate_text_with_chatgpt_async(text: str, filename: str, target_language: str) -> str:
    """Асинхронный перевод файла по чанкам с семафором"""

    chunks = split_into_chunks(text, CHUNK_SIZE, filename)
    file_ext = os.path.splitext(filename)[1].lower()

    # Динамический системный промпт на основе выбранного языка
    system_prompt = f"""
...
КРИТИЧЕСКИ ВАЖНО - ЛОКАЛИЗАЦИЯ ИМЕН И НАЗВАНИЙ:
- ОБЯЗАТЕЛЬНО замени все имена людей на типичные {target_language} имена
- ОБЯЗАТЕЛЬНО замени фамилии на {target_language} аналоги
- ОБЯЗАТЕЛЬНО замени названия городов на {target_language} или соответствующие культуре города
- ОБЯЗАТЕЛЬНО замени названия компаний на {target_language} или локальные аналоги
- ОБЯЗАТЕЛЬНО замени атрибут lang в HTML: <html lang="**"> → правильный код языка для {target_language}
- НИКОГДА не оставляй оригинальные англоязычные имена - всегда находи {target_language} эквивалент
- Если прямого аналога нет - используй наиболее подходящее {target_language} или культурно-соответствующее название
- НЕ упоминай о том, что ты что-то заменил - просто делай это

Примеры локализации для {target_language}:
- Имена: John/Jane → найди типичные {target_language} имена
- Фамилии: Smith/Johnson → найди распространенные {target_language} фамилии
- Города: New York/London → найди крупные города где говорят на {target_language}
- Компании: заменяй на местные или международные бренды
...
ОБЯЗАТЕЛЬНО замени на {target_language} аналоги:
- Все имена людей на типичные {target_language} имена
- Все фамилии на распространенные {target_language} фамилии
- Все города на крупные города где говорят на {target_language}
- Любые компании и бренды на {target_language} или местные аналоги
...
ОБЯЗАТЕЛЬНО замени все имена на {target_language}:
- Все имена людей на типичные {target_language} имена
- Все фамилии на распространенные {target_language} фамилии
- Все города на крупные города где говорят на {target_language}
- Компании на {target_language} или местные аналоги
...
- Все фамилии на распространенные {target_language} фамилии
- Все города на крупные города где говорят на {target_language}
- Все компании на {target_language} или местные аналоги
...
- ОБЯЗАТЕЛЬНО замени все имена на типичные {target_language} имена
- ОБЯЗАТЕЛЬНО замени фамилии на распространенные {target_language} фамилии
- ОБЯЗАТЕЛЬНО замени города на крупные города где говорят на {target_language}
- замени все компании на {target_language} или местные аналоги
...
def translate_text_with_chatgpt(text: str, filename: str, target_language: str) -> str:
    return asyncio.run(
        translate_text_with_chatgpt_async(text, filename, target_language)
    )


async def process_translation_in_background(landing_id: str, target_language: str, message: Message, status_msg: Message):
...
async def process_language_choice(message: Message, state: FSMContext):
    ...
    asyncio.create_task(process_translation_in_background(landing_id, target_language, message, status_msg))
